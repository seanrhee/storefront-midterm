...
const getInboxForUser = (id) => {
  return db.query(`
  SELECT messages.id as message_id, users.id, CONCAT(first_name, ' ', last_name) as full_name, items.photo_url, items.owner_id, items.price_per_item, messages.message, messages.buyer_id, messages.seller_id, messages.date_sent
  FROM users
  JOIN items ON users.id = items.owner_id
  JOIN messages ON users.id = messages.seller_id
